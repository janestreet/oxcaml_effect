...
type 'a op2 = Print : string -> unit op2
...
let handle_state (* local_ *) hs init f =
  let rec handle (state : int) = function
    ...
    | Eff1.Operation (Get, k) -> handle state (Handled_effect.continue k state hs)
    | Eff1.Operation (Set new_state, k) ->
      handle new_state (Handled_effect.continue k () hs)
  in
...
let handle_print (* local_ *) hs f =
  let rec handle = function
    ...
    | Eff2.Operation (Print s, k) ->
      print_string s;
      handle (Handled_effect.continue k () hs)
  in
